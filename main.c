...
void gerarPalavra() {
  ...
  char palavras[50][11] = {"lua",   "abacaxi",  "planta", "touca",   "lixo",
                           "livro", "cachorro", "fruta",  "nicolas", "manta"};

  strcpy(palavraEscolhida, palavras[rand() % 10]);
}
...
int main() {
  ...
  while (erros < VIDAS) {
    ...
    if (valor)
      puts("\nEssa letra esta presente na palavra...");
    else {
      puts("\nEssa letra NAO esta presente na palavra...");
      erros++;
    } if (verificar()) {
      fim(1);
      break;
